...
class BasketTests(unittest.TestCase):
	...
	def setUp(self):
		self.keijon_ostoskori = Basket("Keijo", ["kissa","pasi","Lada 1200"], 30, 0.24, 3, 10)
	...
	# Let's test if alv is a number
	def test_is_alv_number(self):
		self.assertTrue(isinstance(self.keijon_ostoskori.alv, numbers.Number), "variable alv should be a number")
		
	# Let's test if variable averagePrice is a number
	def test_is_averagePrice_number(self):
		self.assertTrue(isinstance(self.keijon_ostoskori.averagePrice, numbers.Number), "variable averagePrice should be a number")
	
	# Let's test if productAmount is a number
	def test_is_productAmount_number(self):
		self.assertTrue(isinstance(self.keijon_ostoskori.productAmount, numbers.Number), "variable productAmount should be a number")
	...
	def test_count_discount(self):
		self.assertEqual(self.keijon_ostoskori.count_discount_price(10), 27.0, "count_discount_price does not count correct value")
		
	# Let's test does count_price_without_alv method work
	def test_count_alv_price(self):
		self.assertEqual(self.keijon_ostoskori.count_price_without_alv(10), 22.8, "count_price_without_alv does not count correct value")
		
	# Let's test does count_productAmount method work
	def test_count_productAmount(self):
		self.assertEqual(self.keijon_ostoskori.count_productAmount(10), 3, "count_productAmount does not count correct value")
		
	# Let's test if average price calculate works
	def test_count_average_price(self):
		self.assertEqual(self.keijon_ostoskori.count_average_price(10), 10, "count_average_price does not count it correctly")
